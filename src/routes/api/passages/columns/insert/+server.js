...
export const POST = async ({ request }) => {
	try {
		...
		const { passageId, insertionWordId } = await request.json();

		// Validate inputs
		if (!passageId || !insertionWordId) {
			return json({ error: 'Missing required fields: passageId and insertionWordId' }, { status: 400 });
		}

		// Perform the column insertion
		await insertColumn(db, session.user.id, passageId, insertionWordId);
