...
- `npm run probe:xpj` (34 assertions) — the cross-part Join Segment: content folded, ranges moved,
  `cachedText` invalidated on both sides, connections not orphaned, nothing lost to cascade.
...
⚠️ **Still unexercised against a database:** **Split Part**, and the section/column granularities of
the cross-part join. Split Part is the remaining operation that writes ranges and moves structure in
one transaction, so it is the next probe worth writing.
